...
@implementation JYCalendarLogic
{
    NSDate *today;//今天的日期
    NSDate *before;//之后的日期
    NSDate *select;//选择的日期
...
- (NSMutableArray *)getCalendarDataSource:(NSDate *)date selectedDate:(NSDate *)selectedDate needDays:(int)days
{
    ...
    today = date;
    before = [date dayInTheFollowingDay:days];
    select = selectedDate;
    
    NSDateComponents *todayDC= [today JYComponents];
    
    NSDateComponents *beforeDC= [before JYComponents];
    ...
    NSMutableArray *calendarMonth = [NSMutableArray array];
    
    for (int i = 0; i <= months; i++) {
        NSDate *month = [today dayInTheFollowingMonth:i];
        NSMutableArray *calendarDays = [NSMutableArray array];
        [self calculateDaysInPreviousMonth:month andArray:calendarDays];
        [self calculateDaysInCurrentMonth:month andArray:calendarDays];
...
- (void)calculateDaysInPreviousMonth:(NSDate *)date andArray:(NSMutableArray *)array
{
    NSInteger weeklyOrdinality = [[date firstDayOfCurrentMonth] weeklyOrdinality];
    NSDate *dayInPreviousMonth = [date dayInThePreviousMonth];
    NSInteger days = [dayInPreviousMonth numberOfDaysInCurrentMonth];
    NSInteger partialDays = weeklyOrdinality -1;
    NSDateComponents *components = [dayInPreviousMonth JYComponents];
    for (int i = (int)(days - partialDays + (NSInteger)1); i < days +1; ++i) {
...
- (void)calculateDaysInCurrentMonth:(NSDate *)date andArray:(NSMutableArray *)array
{
    NSInteger days = [date numberOfDaysInCurrentMonth];
    NSDateComponents *components = [date JYComponents];
...
- (void)calculateDaysInFollowingMonthWithDate:(NSDate *)date andArray:(NSMutableArray *)array
{
    ...
    NSUInteger partialDays = 7 - weeklyOrdinality;
    NSDateComponents *components = [[date dayInTheFollowingMonth] JYComponents];
    
    for (int i = 1; i < partialDays ; ++i) {
        CalendarDayModel *calendarDay = [CalendarDayModel calendarDayWithYear:components.year month:components.month day:i];
        calendarDay.style = CellDayTypeEmpty;
        [array addObject:calendarDay];
    }
...
- (void)changeStyle:(CalendarDayModel *)calendarDay
{
    ...
    NSDateComponents *calendarToDay  = [today JYComponents];
    NSDateComponents *calendarbefore = [before JYComponents];
    NSDateComponents *calendarSelect = [select JYComponents];
